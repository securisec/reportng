...
class Reportng:
    ...
    def section(
        ...
        Args:
            ...
            append_to_previous (bool, optional): Add as extra data to the previous container. Defaults to False.
        ...
        style = self._append_section(append_to_previous)
    ...
    def code(
        ...
        with tag.div(
            ...
            if add_alert:
                rng.HelperFunctions.make_alert(
                    (add_alert.get("color"), add_alert.get("message"),)
                )
            with tag.div(
                _class="container",
    ...
    def table(
        ...
        if not isinstance(data, list):
            raise TypeError("Not a list of data")
        # Saves length of first arg
        check_length = len(data[0])
        # Run checks agains header
        if not isinstance(table_header, list):
            raise TypeError("Headers are not a list")
                ...
                with tag.table(
                    ...
                    for r in range(len(data)):
                        with tag.tr():
                            ...
                            if len(data[r]) != check_length:
                                raise rng.TableError(
                                    "Length of all data sets has to be the same"
                                )
        ...
        with tag.div(
            ...
            with tag.div(_class="row justify-content-center"):
                for i in range(len(cards)):
                    ...
                    if len(cards[i]) < 2 or len(cards[i]) > 3:
